...
//
// mcp25xxfd.c - Microchip MCP25xxFD Family CAN controller driver
//
...
static void mcp25xxfd_dump_reg_osc(const struct mcp25xxfd_priv *priv, u32 val, u16 addr)
{
	...
	__dump_mask(val, MCP25XXFD_OSC, CLKODIV, "0x%02lu", "Clock Output Divisor");
...
static void mcp25xxfd_dump_reg_fifosta(const struct mcp25xxfd_priv *priv, u32 val, u16 addr)
{
	...
	__dump_bit(val, MCP25XXFD_CAN_FIFOSTA, TXABT, "Message Aborted Status (0=completed successfully, 1=aborted)");
...
do { \
	mcp25xxfd_dump_reg_##val(priv, (regs)->val, \
				 (u16)(offsetof(typeof(*(regs)), val) + \
				       (sizeof(*(regs)) == sizeof(struct mcp25xxfd_dump_regs) ? 0 : MCP25XXFD_OSC))); \
	pr_info("\n"); \
} while (0)
